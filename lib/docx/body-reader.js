...
var preserverUtils = require("../styles/preserverUtils");
...
function BodyReader(options) {
    ...
    function readRunProperties(element) {
        return readRunStyle(element).map(function(style) {
            ...
            return {
                ...
                color: element.firstOrEmpty("w:color").attributes["w:val"], // [Moore] add fontColor
                highlight: element.firstOrEmpty("w:highlight").attributes["w:val"], // [Moore] add backgroundColor
            };
        });
    }
    ...
    var xmlElementReaders = {
        ...
        "w:pPr": function(element) { // 标记包括直接应用的段落格式的属性，如段落之前或之后的空格、段落对齐方式或缩进
            return readParagraphStyle(element).map(function(style) {
                // console.log('body-reader--w:pPr', element)
                let sectPr = element.first('w:sectPr');
                let pageInfo;
                // if (sectPr && !sectPr.attributes['w:rsidSect']) { // 页面信息，发现会有很多sectPr标记，但不需要分页，目前按w:rsidSect特殊处理
                //     let size = sectPr.firstOrEmpty("w:pgSz").attributes;
                //     let width = (+size["w:w"]) / 1440 * 2.54;
                //     let height = (+size["w:h"]) / 1440 * 2.54;
        ...
        /** [Moore] add pageInfo
         *  此标记包括页面大小、边距、方向等设置,单位是缇（dxa）
         *  @see https://blog.csdn.net/MooreLxr/article/details/120859899
         */
        "w:sectPr": function (element) {
            // console.log('body-reader--w:sectPr', element)
            const size = element.firstOrEmpty("w:pgSz").attributes;
            const marg = element.firstOrEmpty("w:pgMar").attributes;
            
            // size中的数值单位是dxa
            const width = (+size["w:w"]) / 1440 * 2.54; // 单位：cm
            const height = (+size["w:h"]) / 1440 * 2.54;
            const marginLeft = (+marg["w:left"]) / 1440 * 2.54;
            const marginTop = (+marg["w:top"]) / 1440 * 2.54;
            const marginRight = (+marg["w:right"]) / 1440 * 2.54;
            const marginBottom = (+marg["w:bottom"]) / 1440 * 2.54;
            const marginHeader = (+marg["w:header"]) / 1440 * 2.54;

            let node = preserverUtils.currentDocumentStyle.pageInfo = {
                type: 'page',
                properties: {
                    width,
                    height,
                    margin: { marginLeft, marginTop, marginRight, marginBottom, marginHeader }
                }
            }
    ...
    function readBlip(element, blip) {
        ...
        var extent = element.first("wp:extent").attributes; // [Moore] add read image size
    ...
    function readImage(imageFile, altText, extent) {
        ...
        var image = documents.Image({
            ...
            extent: extent // [Moore] add image size
        });
